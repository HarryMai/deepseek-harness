...
请先按照[根目录 README](../../../README.zh.md#run) 中的说明启动 Web UI。浏览器命令会打印访问地址；桌面命令会在 Electron 中自动打开同一套 UI。本指南从界面已经打开的状态开始。`dsh` 进程会把启动时所在的目录作为默认文件系统位置；全新的 Web UI 则不会选中任何工作区，你需要添加一个工作区。
