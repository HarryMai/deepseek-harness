...
Start the Web UI through the [root README](../../../README.md#run). The browser command prints its URL; the desktop command opens the same UI in Electron automatically. This guide begins after the interface is open. The `dsh` process uses its invoking directory as the default filesystem location, but a fresh Web UI has no selected workspace until you add one.
