...
async function main(): Promise<void> {
  ...
  const development = !app.isPackaged
  const developmentRuntime = developmentProject() ?? (development ? resources.dsh : undefined)
  const activeProject = paths.profile
  const managerRuntime = developmentRuntime === undefined ? resources : { ...resources, dsh: developmentRuntime }
  const manager = new DesktopProjectManager(paths, managerRuntime)
  let quitting = false
  ...
  const backend = new DesktopBackendController((onFailure) => {
    ...
    const host = new DesktopHostProcess(resources.node, developmentRuntime ?? resources.dsh, activeProject,
