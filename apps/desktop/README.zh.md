...
该命令按常规规则解析 Harness home，默认使用 `~/.dsh`，因此会话、设置、凭据和 MCP 配置都使用同一个目录；显式 `DSH_HOME` 可以替换该默认值。Electron 继续使用平台默认的用户数据位置，Renderer DevTools 保持关闭，也不会启用任何进程调试器。每次启动使用独立的临时 npm 项目，同时提供 Host 运行时和 Web 资源，并在 Electron 退出后将其删除；执行前必须已有构建产物。
...
每条打包命令都会检查所提供的应用 ID 和 npm registry、平台打包设置，并探测本次运行要用的外部工具：归档读取工具，以及 Windows 目标的安装器编译器。已选择证书的发布还会在构建与下载前检查应用 ID、策略和更新地址，以及该模式需要的签名配置。macOS 检查身份、Team ID、一套完整公证凭据、`CSC_LINK` 指定的可读本地 p12 文件、显式配置的 `CSC_KEY_PASSWORD`，以及引用的 API Key 和钥匙串文件；Windows 检查公开代码签名证书、SignTool 文件、容器名称和 PIN 格式。未选择证书或设置 `DSH_DESKTOP_UNSIGNED=1` 时，若提供应用 ID 则会校验其格式，但不要求签名凭据、策略配置或更新地址；未签名产物省略签名、公证、更新元数据和发布完成记录。仅准备 Windows 资源不要求签名凭据。配置检查不验证 PIN 是否正确、Token 是否登录、钥匙串是否解锁或 Apple 是否接受凭据；实际签名与公证负责这些检查。`--build-version auto` 会访问目标 bucket，`--check` 下同样如此。单独运行相同检查：
...
该命令需要常规构建依赖，包括编译原生模块所需的 Python 和 Visual C++ 构建工具。Python 不在 `PATH` 中时，将 `PYTHON` 设置为其可执行文件路径；发布字段只需一个其余内容为空的本地 `.env.windows`。命令将安装包写入 `.desktop-build/targets/win-x64/unsigned-artifacts/`，省略自动更新配置，清除签名凭据，且不生成发布完成记录。它不需要应用 ID、EV 凭据、策略或更新源地址。签名打包和上传命令仍遵循正式发布要求。
