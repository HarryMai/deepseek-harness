...
A certificate-selected package checks the application ID, policy and update origins, and mode-specific signing configuration before building or downloading. macOS checks the identity, Team ID, one complete notarization strategy, readable local `CSC_LINK` p12 file, explicit `CSC_KEY_PASSWORD`, and referenced API key and keychain files; Windows checks the public code-signing certificate, SignTool file, container name, and PIN format. Without a selected certificate or with `DSH_DESKTOP_UNSIGNED=1`, any supplied application ID is validated but no application ID, policy, update origin, or signing credential is required; the output omits signing, notarization, updater metadata, and a release completion record. Configuration checks do not authenticate the PIN, log in to the token, unlock a keychain, or contact Apple; actual signing and notarization perform those checks. Run the same checks separately:
...
The command requires the normal build dependencies, including Python and Visual C++ build tools for native modules. Set `PYTHON` to the Python executable when it is absent from `PATH`; an otherwise empty local `.env.windows` is sufficient for release fields. It writes the installer to `.desktop-build/targets/win-x64/unsigned-artifacts/`, omits automatic-update configuration, strips signing credentials, and creates no release completion record. It does not require an application ID, EV credentials, a policy, or an update origin. The signed packaging and upload commands retain their release requirements.
