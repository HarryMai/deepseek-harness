...
const environment = { DSH_DESKTOP_APP_ID: 'com.example.test', DSH_DESKTOP_AUTO_UPDATE_ENV: 'test',
  ...
  DSH_DESKTOP_WINDOWS_CER_FILE: 'fixture.cer', DSH_DESKTOP_WINDOWS_TOKEN_PIN: 'fixture-pin',
  DSH_DESKTOP_WINDOWS_SIGNATURE_CACHE_CONCURRENCY: '2' }
const macEnvironment = { ...environment, DSH_DESKTOP_MACOS_SIGNING_IDENTITY: 'Example Company (TEAMID1234)',
...
it('checks the assembled macOS runtime before notarizing and recording the release', async () => {
  ...
  await packageTarget(parseDesktopPackageInvocation(['mac-arm64'], 'darwin', 'arm64'), macEnvironment, run)
...
it.each([false, true])('scopes the Apple proxy around notarization (directory=%s)', async (directory) => {
  ...
  await packageTarget(parseDesktopPackageInvocation(['mac-arm64', ...(directory ? ['--dir'] : [])], 'darwin', 'arm64'), {
    ...macEnvironment, APPLE_KEYCHAIN_PROFILE: 'fixture', DSH_DESKTOP_MACOS_NOTARIZATION_PROXY: 'http://apple.example:8081',
  }, run)
...
it('does not write a release completion record when Apple proxy cleanup fails', async () => {
  ...
  await expect(packageTarget(parseDesktopPackageInvocation(['mac-arm64'], 'darwin', 'arm64'), macEnvironment, run))
