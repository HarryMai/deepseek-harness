...
describe('Desktop local packaging configuration', () => {
    ...
    await withDirectory(async (directory) => {
      ...
      expect(() => {
        validateDesktopPackageEnvironment({
          ...missing,
          ...(platform === 'win32' ? WINDOWS_SIGNING : MAC_IDENTITY),
  ...
  it('checks application and update configuration before Windows credentials while preserving unsigned and preparation modes', () => {
    expect(() => { validateDesktopPackageEnvironment({}, WINDOWS, { unsigned: true }) }).not.toThrow()
    expect(() => {
    ...
    expect(() => { validateDesktopPackageEnvironment(RELEASE, WINDOWS) }).not.toThrow()
    expect(() => {
    ...
    expect(() => {
      validateDesktopPackageEnvironment({
        ...POLICY,
        ...WINDOWS_SIGNING,
        DSH_DESKTOP_APP_ID: RELEASE.DSH_DESKTOP_APP_ID,
  ...
  it('rejects incomplete macOS identity and credentials and checks referenced files without contacting Apple', async () => {
    expect(() => { validateDesktopPackageEnvironment(RELEASE, MACOS) }).not.toThrow()
    expect(() => {
