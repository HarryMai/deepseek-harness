...
describe('desktop macOS release signature', () => {
  ...
  it('builds an unsigned macOS package when no signing identity is selected', async () => {
    const { createElectronBuilderConfig } = await import('../electron-builder.config.mjs')
  ...
  it('builds an unsigned Windows package when no certificate is selected', async () => {
    const { createElectronBuilderConfig } = await import('../electron-builder.config.mjs')
    const config = createElectronBuilderConfig({
    ...
    expect(config).toMatchObject({
      win: { forceCodeSigning: false, signExecutable: false, signtoolOptions: null },
  ...
  it('supports explicit unsigned macOS builds and rejects malformed signing modes', async () => {
    const { createElectronBuilderConfig } = await import('../electron-builder.config.mjs')
    const config = createElectronBuilderConfig({
      ...RELEASE_ENVIRONMENT,
      DSH_DESKTOP_UNSIGNED: '1',
    }, 'darwin', 'arm64')
    expect(config.mac).toMatchObject({ forceCodeSigning: false, notarize: false, sign: null })
