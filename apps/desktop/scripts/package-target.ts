...
async function main(): Promise<void> {
  ...
  const run = target.platform === 'win32'
    ? createPackagingRun(join(APP_ROOT, '.desktop-build', 'packaging-runs'), {
      target: target.name, unsigned: !signed, version: packageVersion(join(APP_ROOT, 'package.json'), 'desktop package'),
    }) : undefined
  if (run !== undefined) console.log(`DESKTOP_PACKAGING_RECORD ${run.directory}`)
  let success = false
  try {
    if (target.platform === 'darwin' && signed) {
...
export async function packageTarget(
  ...
  const electronBuilderEnv = desktopElectronBuilderEnvironment(targetEnv, unsigned)
