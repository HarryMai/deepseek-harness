...
/**
 * Report whether the selected target has the certificate selector that enables signing.
 * @param target - Target selected for packaging.
 * @param environment - Platform-local packaging environment.
 * @returns Whether this target follows the signed release path.
 */
...
async function resolveRequestedBuildVersion(
  ...
  return suggestDesktopBuildVersion({
    ...
    artifactsRoot: unsigned ? paths.unsignedArtifacts : paths.artifacts,
  })
}
...
async function main(): Promise<void> {
  ...
  const signed = !invocation.unsigned && isDesktopPackageSigned(target, environment)
  ...
  const buildVersion = await resolveRequestedBuildVersion(invocation, productVersion, environment, unsigned)
  environment[DESKTOP_BUILD_VERSION_ENV] = buildVersion
  if (invocation.check) {
    validateDesktopPackageEnvironment(environment, target, { ...invocation, unsigned })
  ...
  let success = false
  try {
    await packagingStep(run.directory, 'configuration', async () => { validateDesktopPackageEnvironment(environment, target, { ...invocation, unsigned }) }, secrets)
    ...
    if (target.platform === 'darwin') {
      ...
      await packagingStep(run.directory, 'macos-package', () => signed
...
export async function packageTarget(
  ...
  if (target.platform === 'darwin' && !invocation.directory && signed) {
    await execute([
