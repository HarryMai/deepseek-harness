...
import { createWindowsTokenSigner, hasWindowsSigningCertificate } from './windows-sign.mjs'
...
const AMBIENT_RELEASE_SETTING = /^(?:DSH_DESKTOP_(?:APP_ID|AUTO_UPDATE_ENV|NPM_REGISTRY|MANDATORY_UPDATE_.*|WINDOWS_.*|MACOS_.*)|APPLE_.*|(?:WIN_)?CSC_.*|DOWNLOAD_(?:TEST|PROD)_.*)$/iu
...
export function validateDesktopPackageEnvironment(environment, target, options = {}) {
  const signed = !options.unsigned && (target.platform === 'win32'
  ...
  if (signed) {
    ...
    resolveOptionalDesktopAppId(environment)
  }
  ...
  if (target.platform === 'win32') {
    ...
    resolveOptionalMacOSSigningEnvironment(environment)
    const strategies = [
