...
> 分工线：本篇 = 分层模型 + 通道无关的 RPC 协议；协议的 Web 实现由 HTTP 上行加 [WebSocket 下行载体](2026-08-04-websocket-downlink-carrier.zh.md)组成，浏览器对象层见 [Web 客户端架构笔记](2026-07-19-gui-web-client-architecture.zh.md)，随附 Electron 的进程模型见[桌面壳说明](2026-08-14-electron-shell-over-web-profile.zh.md)。
...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop`（`@deepseek-ai/dsh-desktop`）在普通 Node 子进程中运行现有 Web profile，并在启用 sandbox 的 Electron renderer 中加载其回环 URL；IPC 只负责进程生命周期。直接 IPC fetch 载体仍是可选的未来传输方式。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 浏览器 renderer 访问，包括随附 Electron 壳的回环访问；零 workspace 依赖（注册表经结构注入到达） |
...
| 应用 | `@deepseek-ai/dsh`（apps/cli）+ `dsh-web-frontend`（apps/web，vite 应用）+ `@deepseek-ai/dsh-desktop`（apps/desktop） | bin 粗分发 + 每个应用一个拼装模块（web.ts / headless.ts）；vite 应用是 `dsh-client-web` 壳表面之上的薄 main；桌面应用拥有 Electron 与普通 Node Host 子进程 | 各应用通过动态 import 或进程隔离，绝不加载彼此的平台运行时；dist 定位等 workspace 知识留在 app |
...
1. **选 fetch 伪造方式**：浏览器同源 HTTP，包括 Electron renderer 的回环访问 / 进程内 `host.handler.fetch` 注入 / 自写传输切面子类（如未来的 Electron IPC 载体，见下文「子类表」）。
2. **在 `apps/` 下写拼装模块**：`startHost()` + 客户端子类 + 该应用私有的信号/打印/退出语义；混合体不建包，拼装写在 app 里。
3. **需要 HTTP 承载才 import `dsh-host-webserver`**，包括回环桌面 renderer；否则使用零端口。
...
| IPC 桥子类（假想示例——随附壳使用回环 HTTP） | Electron 壳 | IPC 序列化往返 | 只需换 doFetch，约定/基类零改 |
