...
The packaged seed is an offline installation kit, not an executable dsh tree. Each `mac-arm64`, `mac-x64`, and `win-x64` target owns its packed packages, runtime, package set, seed, pnpm preparation state, and unpacked application under `.desktop-build/<target>`, while electron-builder writes every installer artifact and channel metadata file to `apps/desktop/out`. The immutable, checksum-verified Node.js download cache is shared. The release build requires matching Electron, root dsh, and private Host versions, builds final npm tarballs from the official source, and verifies the private Host tarball contains `lib/index.js` and `config/desktop.cordis.patch.yml`. Bundled pnpm materializes the target-specific production dependency graph, validates the local first-party package mappings, creates deterministic seed store shards, and records their integrity. When the build host cannot execute the target Node.js runtime, offline verification keeps lifecycle scripts disabled; the target runtime runs them during its first profile installation.
...
`DSH_DESKTOP_AUTO_UPDATE_ENV` selects the test deployment by default or production for the target-specific generic-provider URL and COS destination. Packaging resolves only the public updater URL, disables direct electron-builder publishing, and removes COS credentials from subprocesses. A signed package writes its completion record only after electron-builder and every signing or notarization hook succeeds; an unsigned package writes no record and cannot be uploaded. Target upload validates that record, both package versions, channel metadata, artifact names, sizes, and SHA-512 values before it reads credentials or sends data.
...
A non-empty `DSH_DESKTOP_MACOS_SIGNING_IDENTITY` selects macOS signing. With that selector, configuration requires the application ID, expected Team ID, and one complete notarytool credential strategy; seed preparation signs every embedded Mach-O object with the selected authority, timestamp, and hardened runtime; electron-builder verifies, notarizes, and staples the application and DMG. Without the selector, the same configuration explicitly disables certificate discovery, signing, notarization, publication metadata, and the release hooks.

A non-empty `DSH_DESKTOP_WINDOWS_CER_FILE` selects Windows EV signing. With that selector, the configured SafeNet-compatible SignTool, public certificate, private-key container, and Token Password are required, and every produced executable receives the hardware-backed signature. Without the certificate selector, the shared packaging configuration disables Windows signing, differential update metadata, and the upload completion record.

All Desktop package commands use one target preparation layout and one electron-builder configuration. Certificate availability selects signed or unsigned behavior without selecting another command or output directory. `pnpm desktop:make:mac` builds macOS arm64 and x64 on a macOS build host, and Windows supports x64 on a Windows x64 host.
...
| Qualification | A non-empty macOS identity or Windows certificate selects the corresponding signing, verification, and update-metadata path. Missing selectors produce unsigned installers and no upload completion record. Update hosting, previous-version installed-artifact tests, and platform GUI recordings remain release-environment gates. |
| Installer output | Every target writes installer artifacts to `apps/desktop/out`; only target preparation remains under `.desktop-build/<target>`. |

`pnpm desktop` starts existing workspace artifacts through a unique disposable project, resolves the normal shared Harness home with `~/.dsh` as its default, leaves Electron at its platform user-data location, keeps Renderer DevTools closed, and does not enable process debuggers. `dev:desktop` builds the current workspace, projects the built CLI and private Desktop Host packages plus their dependency links into a disposable project, uses an isolated Harness home, opens the Main, Renderer, and Host debuggers, and starts unpackaged Electron without preparing release resources. Package mutation is disabled in both modes because their linked dependency graph is not a pnpm-installed desktop project. `pnpm desktop:make:mac` passes macOS arm64 and x64 through runtime preparation, seed installation, and electron-builder in sequence on a macOS build host; `pnpm desktop:make:win` passes Windows x64. Each target also has an unpacked-directory variant for release-path verification before installer generation.

`pnpm desktop:make:mac` passes macOS arm64 and x64 through runtime preparation, seed installation, and electron-builder in sequence; `pnpm desktop:make:win` passes Windows x64. Certificate selectors decide signing behavior without changing that path.
...
- A selected Windows signing path requires the validated SignTool, EV token, matching public leaf certificate, Token Password, and explicit key container; it never exports the private key.
- Missing platform certificate selectors produce visibly unsigned installers in the common output directory without an upload completion record.
