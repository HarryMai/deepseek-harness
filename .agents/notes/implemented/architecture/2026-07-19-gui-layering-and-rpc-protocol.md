...
> Division of labor: this document = the layering model + the channel-independent RPC protocol; the protocol's Web implementation combines HTTP uplink with the [WebSocket downlink carrier](2026-08-04-websocket-downlink-carrier.md), while the browser object layer is in the [web client architecture note](2026-07-19-gui-web-client-architecture.md) and the shipped Electron process model is in the [desktop-shell note](2026-08-14-electron-shell-over-web-profile.md).
...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` (`@deepseek-ai/dsh-desktop`) runs the existing Web profile in an ordinary Node child and loads its loopback URL in a sandboxed Electron renderer; IPC owns process lifecycle only. A direct IPC fetch carrier remains an optional future transport.
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser-renderer access, including the shipped Electron shell over loopback; zero workspace dependencies (the registry arrives by structural injection) |
...
| Application | `@deepseek-ai/dsh` (apps/cli) + `dsh-web-frontend` (apps/web, the vite application) + `@deepseek-ai/dsh-desktop` (apps/desktop) | Coarse bin dispatch + one assembly module per application (web.ts / headless.ts); the vite app is a thin main over the `dsh-client-web` shell surface; the desktop app owns Electron and its ordinary-Node Host child | Applications use dynamic imports or process separation so they never load each other's platform runtime; workspace knowledge like dist location stays in the app |
...
1. **Pick a fetch impersonation**: browser same-origin HTTP, including an Electron renderer over loopback / in-process `host.handler.fetch` injection / your own transport-aspect subclass (e.g. a future Electron IPC carrier, see the "Subclass table" below).
...
3. **Import `dsh-host-webserver` only if you need HTTP carriage**, including a loopback desktop renderer; otherwise use zero ports.
...
| IPC bridge subclass (hypothetical example — the shipped shell uses loopback HTTP) | an Electron shell | IPC serialization round trip | would swap only doFetch; contract and base class unchanged |
