...
打包 seed 是离线安装包，而不是可执行 dsh 目录。每个 `mac-arm64`、`mac-x64` 和 `win-x64` 目标都在 `.desktop-build/<target>` 下持有自己的打包输入、运行时、包集合、seed、pnpm 准备状态和未打包应用，而 electron-builder 会将所有安装包产物和频道元数据写入 `apps/desktop/out`。不可变且经过校验和验证的 Node.js 下载缓存保持共享。发布构建要求 Electron、根 dsh 与私有 Host 使用匹配版本，从正式源码构建最终 npm tarball，并验证私有 Host tarball 包含 `lib/index.js` 与 `config/desktop.cordis.patch.yml`。内置 pnpm 会物化目标专用的生产依赖图，验证本地第一方包映射，创建确定性的 seed store 分片并记录完整性。当构建宿主不能执行目标 Node.js 运行时时，离线验证会继续禁用生命周期脚本；目标运行时会在首次安装 profile 时执行它们。
...
`DSH_DESKTOP_AUTO_UPDATE_ENV` 默认为测试部署，也可以选择生产部署，并同时决定目标专用的 generic-provider URL 与 COS 目标。打包只解析公开更新 URL、禁止直接 electron-builder 发布，并从子进程环境中删除 COS 凭据。已签名打包只有在 electron-builder 以及每个签名或公证 hook 成功后才写入完成记录；未签名打包不写入记录，也不能上传。目标上传会在读取凭据或发送数据前验证该记录、两个包版本、频道元数据、产物名称、大小与 SHA-512。
...
非空的 `DSH_DESKTOP_MACOS_SIGNING_IDENTITY` 会选择 macOS 签名。存在该选择器时，配置要求应用 ID、预期 Team ID 与一套完整 notarytool 凭据；seed 准备会以选定 Authority、安全时间戳和 hardened runtime 签署每个内嵌 Mach-O 对象；electron-builder 会验证、公证并钉票应用和 DMG。选择器缺失时，同一配置会明确禁用证书发现、签名、公证、发布元数据和发布 hook。

非空的 `DSH_DESKTOP_WINDOWS_CER_FILE` 会选择 Windows EV 签名。存在该选择器时，必须提供已配置的 SafeNet 兼容 SignTool、公开证书、私钥容器和 Token Password，并为每个生成的可执行文件应用硬件支持的签名。证书选择器缺失时，共用打包配置会禁用 Windows 签名、差分更新元数据和上传完成记录。

所有 Desktop 打包命令使用同一套目标准备布局和同一份 electron-builder 配置。证书可用性选择已签名或未签名行为，但不会选择另一条命令或另一处输出目录。`pnpm desktop:make:mac` 在 macOS 构建宿主上构建 macOS arm64 和 x64，Windows 在 Windows x64 宿主上支持 x64。
...
| 资格验证 | 非空的 macOS 身份或 Windows 证书会选择相应的签名、验证和更新元数据路径。选择器缺失时生成未签名安装包，且不写入上传完成记录。更新托管、跨上一版本的已安装产物测试和各平台 GUI 录制仍是发布环境门槛。 |
| 安装包输出 | 所有目标都将安装包产物写入 `apps/desktop/out`；只有目标准备状态保留在 `.desktop-build/<target>`。 |

`pnpm desktop` 通过独立的一次性项目启动 workspace 中已有的构建产物，按常规规则解析共享 Harness home 并默认使用 `~/.dsh`，让 Electron 继续使用平台用户数据位置，保持 Renderer DevTools 关闭，并且不启用进程调试器。`dev:desktop` 会构建当前 workspace，把已构建 CLI 包、私有 Desktop Host 包及其依赖链接投影为一次性项目，使用隔离的 Harness home，打开 Main、Renderer 和 Host 调试器，并在不准备发布资源的情况下启动未打包 Electron。这两种模式的链接依赖图都不是由 pnpm 安装的桌面项目，因此都会禁用包修改。`pnpm desktop:make:mac` 会在 macOS 构建宿主上依次将 macOS arm64 和 x64 传给运行时准备、seed 安装和 electron-builder；`pnpm desktop:make:win` 会传入 Windows x64。每个目标还提供未封装安装器的变体，用于在生成安装器前验证发布路径。

`pnpm desktop:make:mac` 会依次将 macOS arm64 和 x64 传给运行时准备、seed 安装和 electron-builder；`pnpm desktop:make:win` 会传入 Windows x64。证书选择器决定签名行为，但不会改变这条路径。
...
- 已选择的 Windows 签名路径要求已验证的 SignTool、EV Token、匹配的公开叶证书、Token Password 和明确的密钥容器，绝不会导出私钥。
- 平台证书选择器缺失时，会在共用输出目录中生成明确的未签名安装程序，且不写入上传完成记录。
