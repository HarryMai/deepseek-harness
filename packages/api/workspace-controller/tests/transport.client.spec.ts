  ...
  it('delivers one baseline followed by increments', async ({ mock, start }) => {
    ...
    const increments: WorkspaceFollowFrame[] = [
      ...
      { type: 'order', workspaceIds: [view.workspaceId] },
      {
        ...
        recycleBinEntries: [{ sessionId: sid('session-one'), archivedAt: '2026-01-01T00:00:00.000Z' }],
      },
    ...
    expect(replaceArchived).toHaveBeenCalledWith({
      ...
      recycleBinEntries: [{ sessionId: 'session-one', archivedAt: '2026-01-01T00:00:00.000Z' }],
    })
...
describe('WorkspaceController', () => {
  ...
  it('publishes the model source and exposes successful Workspace commands', async ({ mock, start }) => {
    const { remote, client } = await gatewayClient(mock, start)
    const model = new ClientWorkspaceModel(remote.workspace)
    model.replaceBaseline({ items: [workspace('one')], archivedSessionIds: [], recycleBinEntries: [], pinnedSessionIds: [] })
    const controller = new WorkspaceController(client.ctx, model)
    ...
    expect(mock.log.requests('workspace/restoreArchivedSessions')).toEqual([{ sessionIds: ['session'], confirmed: true }])
    expect(mock.log.requests('workspace/clearRecycleBin')).toEqual([{ confirmed: true }])
