...
export function baseline(...ids: readonly string[]): WorkspaceBaselineFrame {
  return {
    type: 'baseline',
    value: { items: ids.map(id => workspace(id)), archivedSessionIds: [], recycleBinEntries: [], pinnedSessionIds: [] },
  }
}
...
export const workspaceWorld: RemoteTable = {
  unary: {
    ...
    'workspace/restoreArchivedSessions': (_request: WorkspaceRestoreArchivedSessionsRequest): RemoteResult<WorkspaceArchiveValue> => ok({
      archivedSessionIds: [], recycleBinEntries: [],
    }),
    'workspace/clearRecycleBin': (_request: WorkspaceClearRecycleBinRequest): RemoteResult<WorkspaceArchiveValue> => ok({
      archivedSessionIds: [], recycleBinEntries: [],
    }),
