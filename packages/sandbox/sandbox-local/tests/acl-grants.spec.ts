...
describe('windows-acl write grants (LocalSandboxProvider)', () => {
  ...
  it('forwards the host debug-log opt-in onto the runner argv in both modes, and omits it unset', async () => {
    ...
    try {
      ...
      const ws = workspaceRoot()
      scratch.push(ws)
      const readOnly = await sandbox.confine(['true'], { mode: 'read-only', workspaceRoot: ws })
      expect(flag(readOnly.argv, '--debug-log')).toBe(logPath)
      const write = await sandbox.confine(['true'], { mode: 'workspace-write', workspaceRoot: ws, sessionId: SessionId('sess-debug') })
      ...
      expect(flag((await sandbox.confine(['true'], { mode: 'read-only', workspaceRoot: ws })).argv, '--debug-log')).toBeUndefined()
