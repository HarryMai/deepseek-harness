...
/**
 * Decode a NUL-terminated UTF-16 string at a native pointer.
 * @param ptr - pointer to the first UTF-16 code unit.
 * @returns the decoded string without its terminator.
 */
export function decodeString16(ptr: NativePtr): string {
  const koffi = requireKoffi()
  const units: number[] = []
  for (let offset = 0; offset < 512; offset += 2) {
    const unit = koffi.decode(ptr, offset, 'uint16') as number
