...
- **可复用的 POSIX 进程检查**：`@deepseek-ai/dsh-subprocess-local/process-inspector` 向需要在本 provider 之外快照并发送信号给后代进程的生命周期所有者公开带身份校验的 Linux／macOS 进程树检查器。Windows 生命周期所有者改用操作系统提供的进程树能力。
