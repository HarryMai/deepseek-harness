...
回收站设置区位于自定义配置之后。它列出活动可恢复归档、接受正整数天的保留期设置，并在恢复一条或多条所选 Session 或清空活动条目前要求 `RiskConfirmation`；清空或到期只会移除产品恢复能力，不删除 Session 历史。
...
- **没有 Session 删除或彻底清除归档**：会话可以归档但绝不会被删除；回收站设置区只能恢复活动条目，清空或到期会删除回收站记录，但仍保留 Session 的归档标记和历史。已归档视图与搜索结果仍可单独取消归档，删除 Workspace 注册记录不会删除 Session。
...
**运行时不变式：** 不发布伴生入口。client 入口注册 Workspace 浏览器、选择器、Session 行操作、对应浮层和回收站设置区。浏览视图状态使用其 slot store，行提示通过注入 hooks 发布，恢复偏好使用 Workspace 回收站命名空间的 Host 共享配置表单。本包不发出 Cordis 事件。
