...
import { RemoteError, TestRemote, stubConfigForm } from '@deepseek-ai/dsh-client-test-runtime'
...
const workspaceState = (
  ...
  items, archivedSessionIds, pinnedSessionIds, recycleBinEntries: [],
...
async function bench() {
  ...
  ctx.provide('workspaces', {
    ...
    restoreArchivedSessions: vi.fn(async () => undefined),
    clearRecycleBin: vi.fn(async () => undefined),
  ...
  const recycleBinScope = stubConfigForm<{ retentionDays: number }>()
  ...
  ctx.provide('configForms', { get: vi.fn(() => recycleBinScope.scope) } as never)
