...
class FakeWorkspaces implements IWorkspaces {
  ...
  onArchive: IWorkspaces['archiveSession'] = async (sessionId) => {
    this.list.update(state => ({
      ...
      recycleBinEntries: [
        ...
        { sessionId, archivedAt: '2026-01-01T00:00:00.000Z' },
      ],
    }))
  }

  onRestore: IWorkspaces['restoreArchivedSessions'] = async (sessionIds) => {
    this.list.update(state => ({
      ...state,
      archivedSessionIds: state.archivedSessionIds.filter(id => !sessionIds.includes(id)),
      recycleBinEntries: state.recycleBinEntries.filter(entry => !sessionIds.includes(entry.sessionId)),
    }))
  }

  onClear: IWorkspaces['clearRecycleBin'] = async () => {
  ...
  onUnarchive: IWorkspaces['unarchiveSession'] = async (sessionId) => {
    this.list.update(state => ({
      ...state,
      ...(state.recycleBinEntries.some(entry => entry.sessionId === sessionId)
        ? {
  ...
  restoreArchivedSessions(sessionIds: readonly SessionId[]): Promise<void> {
    this.restoreCalls.push([...sessionIds])
    return this.onRestore(sessionIds)
  }

  clearRecycleBin(): Promise<void> {
    this.clearCalls.push(this.clearCalls.length + 1)
    return this.onClear()
  }
