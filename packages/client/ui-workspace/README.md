...
The Recycle Bin Settings section follows Custom Configuration. It lists active recoverable archives, accepts a positive whole-day retention preference, and requires `RiskConfirmation` before restoring one or several selected Sessions or clearing the active entries; clearing or expiry removes product recovery without deleting Session history.
...
- **No Session deletion or hard archive purge** — sessions can be archived but never deleted; Recycle Bin Settings restores only active entries, and clearing or expiry removes the Recycle Bin record while leaving the Session archived with its history intact. The archived view and search results still allow individual unarchive, and deleting a Workspace registration never deletes Sessions.
...
**Runtime invariant:** No companion is published. The client entry registers the Workspace browser, picker, Session row actions, their overlays, and the Recycle Bin settings section. Browser view state uses its slot store, row notices use injected hooks, and recovery preferences use the shared Host configuration form for the Workspace recycle-bin namespace. The package emits no Cordis events.
